...
% #   see the COPYING file distributed with CoSMoMVPA.           #
    ...
    for fold=1:npartitions
        ...
        if ~isempty(normalization)
            if ~isempty(opt.normalize) && opt.normalize==3
                [train_data,params]=cosmo_normalize(train_data,normalization);
                test_data=cosmo_normalize(test_data,params); % this uses crossvalidation
            elseif ~isempty(opt.normalize)
                [train_data,params]=cosmo_normalize(train_data,normalization,opt.normalize);
                [test_data,params]=cosmo_normalize(test_data,normalization,opt.normalize);
            else
                [train_data,params]=cosmo_normalize(train_data,normalization);
                [test_data,params]=cosmo_normalize(test_data,normalization);
            end
        end
