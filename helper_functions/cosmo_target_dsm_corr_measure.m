...
% MODIFIED BY MORITZ:
%1: demean conditions
%2: use partitions (for cross RSA or leave-one-out RSA)
%3: custom correlation matrix (not the full lower diagonal but kind of a three-staircase matrix)
% (4: get r square as output; see line 260)
...
if isfield(params,'normalization')
        %ds = cosmo_normalize(ds,'demean',2); % 2 = across voxels (within a condition) --> each condition is demeaned separately
        ds = cosmo_normalize(ds,params.normalization,params.normalize); % 2 = across voxels (within a condition) --> each condition is demeaned separately
end
% if isfield(params,'normalize')
%     if params.normalize==1
%         ds = cosmo_normalize(ds,'demean',2); % across voxels (within a condition) --> each condition is demeaned separately
%     end
% end
...
%% modified 3
if isfield(params,'customMask')
    if params.customMask ==1;
        %modifiedMask == 1
        ds_pdist = cosmo_squareform(ds_pdist);
        msk = logical(zeros(8,8));
        ones_idx = [3 4 5 6 7 8 11 12 13 14 15 16 21 22 23 24 29 30 31 32 39 40 47 48];
        msk(ones_idx)=1;
        ds_pdist=ds_pdist(msk);
        
        has_model_dsms=isfield(params,'glm_dsm');
        if has_model_dsms
            for iRDM = 1:length(params.glm_dsm)
                x = params.glm_dsm{iRDM};
                params.glm_dsm{iRDM} = x(msk);
            end
        else
            params.target_dsm=params.target_dsm(msk);
        end
    else
        % disp('no masking of correlation matrix. Correct?');
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% MODIFIED BY MORITZ
if isfield(params,'rsquare');
    
    [B,BINT,R,RINT,STATS]=regress(ds_pdist_zscore,dsm_mat_zscore);
    ds_sa=struct();
    ds_sa.samples=STATS(1);
    ds_sa.sa.labels={'rsquare'};
    ds_sa.sa.metric=repmat({params.metric},1,1);
