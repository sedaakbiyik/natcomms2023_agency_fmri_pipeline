function create_first_run(iSub)
...
for i =  1
    ...
    if i == 1
        ...
        %for all runs but the first, align the first volume of that run to
        %the first volume of the first run (within-session alignment)
       
        if logical(mod(iSub,2)) %% if odd number, video first, align to video
       
            cfg.preprocessing.motionCorrection.targetFMR = [cfg.copyToTargetFolder '/' sprintf('SUB%02d_01_AGENCY_video.fmr', iSub)]
        
        elseif  ~mod(iSub,2) %% if even number, sentence first, align to sentence
       
           cfg.preprocessing.motionCorrection.targetFMR = [cfg.copyToTargetFolder '/' sprintf('SUB%02d_01_AGENCY_sentence.fmr', iSub)]
